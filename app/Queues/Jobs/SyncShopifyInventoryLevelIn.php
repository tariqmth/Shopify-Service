...
class SyncShopifyInventoryLevelIn implements ShouldQueue, SyncJob
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels, HandleJobExceptions;

    protected $shopifyInventoryItemId;
    protected $data;
    protected $syncer;
    ...
    public function __construct(
        ...
        $this->shopifyInventoryItemId = $shopifyInventoryItem->id;
        $this->data = $inventoryLevelData;
        $shopifyStore = $shopifyInventoryItem->shopifyProductVariant->shopifyProduct->shopifyStore;
        $this->shopifyStoreId = $shopifyStore->id;
        $this->clientId = $shopifyStore->rexSalesChannel->client_id;
        $this->entityExternalId = $shopifyInventoryItem->external_id;
    }
     ...
     * @return void
     */
    public function handle(ShopifyInventoryItemSyncer $syncer)
    {
        try {
            $syncer->performSyncInLevel($this->shopifyInventoryItemId, $this->data);
    ...
    {
        return $this->shopifyInventoryItemId;
    }
