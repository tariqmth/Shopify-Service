...
class FulfilmentOptionsCheckout extends FulfilmentOptions {
  constructor(settings) {
    ...
    this.priorityRadio = '#checkout_shipping_rate_id_shopify-priority20delivery-20_00';
    ...
    this.priorityOrderCutoff=14;
    this.priorityCheckAvailability = 1;
    this.priorityDeliverWithin = 5;
  ...
  addComponents() {
    ...
    this.focContainer.parentNode.insertBefore(this.focSpinner, this.focContainer.nextSibling);
  ...
  setMode(mode) {    
	...
	if (mode === 'pickup') {
      ...
      document.getElementById('foc-results-container').style.display = 'none';
  ...
  updateContinueButton(available) {
    ...
    if (continueButton) {
      if ( ( !available && ( this.mode === "pickup" || this.mode === "priority") ) || this.mode === "load" ) {
...
class CheckoutResults extends Results {
  ...
  update() {
    ...
    Shopify.getCart(function(cart) {
      ...
      if ( parent.foc.getMode() === 'pickup' || parent.foc.priorityOrderCutoff > currentHour ) {  
      	parent.fetch();
      } else {     
      	document.getElementById('foc-spinner').style.visibility = "hidden";
      	document.getElementById('foc-spinner').style.display = "none";
    	document.getElementById('foc-results-container').style.display = '';
        document.querySelector(parent.foc.shippingSectionSelector).style.display = '';
        document.querySelector(parent.foc.priorityRadio).disabled = true;
  ...
  fetch() {
    ...
    fetch(url)
      ...
      .then(function(json) {    
        ...
        } else if (parent.foc.getMode() === 'priority') {
          if (parent.count() < 1) {
            document.getElementById('foc-outlets').style.display='none';
        ...
        } else if (parent.foc.getMode() === 'load') {
          document.querySelector(parent.foc.shippingSectionSelector).style.display = '';
          if (parent.count() < 1 || !parent.areOutletsAvailable() ) {
            document.querySelector(parent.foc.priorityRadio).disabled = true;
            document.querySelector("[data-shipping-method-label-title='Priority Delivery']").innerHTML = parent.foc.priorityLabel+' (Unavailable)';
          }
        }
